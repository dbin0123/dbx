...
use crate::sql_dialect::DialectKind;
...
fn list_dialect_type_names_in(dialect_name: &str, registry: &DialectRegistry) -> Vec<String> {
    let loaded = registry.get(dialect_name).or_else(|| {
        // Public callers also use stable dialect labels such as `postgres`,
        // while the embedded catalog keeps the product name `PostgreSQL`.
        let kind = DialectKind::from_label(dialect_name)?;
        registry.get(core_catalog_name(kind))
    });
