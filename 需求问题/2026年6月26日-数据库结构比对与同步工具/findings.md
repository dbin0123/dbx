...
## V4 设计方案差距分析（2026-06-29 会话）

以下基于 V4 设计文档、任务计划 (task_plan.md) 与实际代码库的三方交叉比对。

### Backend 未实现功能

#### A. 外部 OSC 工具状态探针（设计 §3.7.2）
- **设计要求**：gh-ost 探针（定期查询 `_gho` 表、行数比例、监听回调 URL/日志文件、识别 copy/cut-over/postpone 状态）、pt-osc 状态解析（`_new` 表与 Trigger）、状态桥接到状态持久化层（OSC_COPYING: 45%）、异常触发反向回滚
- **实际状态**：sql_risk.rs 仅有注释提及 pt-osc/gh-ost，无任何探针实现
- **影响**：无法监控外部在线变更工具的执行进度，不能自动桥接回内部状态机

#### B. 方言插件化热加载与 CLI 引导工具（设计 §3.1.3、§4.6）
- **设计要求**：扫描 `plugins/dialects/` 目录动态加载、运行时热加载无需重启、UI 按推荐/兼容/废弃分组、废弃版本二次确认、`dbdiff dialect init` 交互式 CLI 向导
- **实际状态**：方言加载基于编译时枚举，无插件目录扫描、无热加载、无 CLI 向导
- **影响**：新增方言需修改源码重新编译，不能动态扩展

#### C. 企业级配置治理（设计 §3.10）— 阶段 10 整个被跳过
- **设计要求**：审批流集成（关键配置变更需审批、危险 DDL 三级审批）、配置变更审计+版本管理、配置回滚、漂移检测（定时比对各环境哈希、推送告警）、COW 读写分离配置热重载
- **实际状态**：phase 10 标记为"跳过"，所有子任务 pending
- **影响**：缺少企业级配置治理，无法满足审批管控、审计追溯、跨环境配置一致性要求

#### D. 状态机缺少 OSC_SYNCING / 部分回滚状态（设计 §3.8.3）
- **设计要求**：PENDING → RUNNING → OSC_SYNCING → SUCCESS/FAILED → ROLLING_BACK → PARTIALLY_ROLLED_BACK/FULLY_ROLLED_BACK/RECOVERY_REQUIRED
- **实际状态**：7 种状态（Created/Running/Paused/Completed/Failed/Cancelled/RolledBack）缺少 OSC_SYNCING、PARTIALLY_ROLLED_BACK、FULLY_ROLLED_BACK、RECOVERY_REQUIRED
- **影响**：无法区分回滚完成度，缺少灾难恢复状态标注

#### E. Prometheus 指标未覆盖 OSC 探针/标签阻断/Trace 丢弃（设计 §5）
- **设计要求**：回滚触发次数、OSC 探针延迟、标签阻断次数、Trace 丢弃数
- **实际状态**：risk_metrics.rs 有 DegradationMetrics，但缺少 OSC/tag/trace 专项指标
- **影响**：可观测性不完整

#### F. 配置引擎 COW 热重载（设计 §3.10）
- **设计要求**：COW 读写分离快照，热重载全程无锁
- **实际状态**：无实现
- **影响**：配置变更需要重启或手动刷新
...
#### G. 标签管理面板
- **设计要求**：标签生命周期管理、白名单、批量导入、过期提醒（设计 §3.2.2）
- **实际状态**：无任何标签管理 UI 组件

#### H. 风险矩阵展示
- **设计要求**：多维可视化风险矩阵，消耗 backend ImpactReport
- **实际状态**：仅有一个简单的 risk badge（计数 compatibility warnings），ImpactReport 未消耗

#### I. 回滚预览面板（并列对比）
- **设计要求**：Forward SQL 与 Rollback SQL 并列展示对比（设计 §2.1 报告渲染器）
- **实际状态**：仅 SQL 文本切换 toggle，无并列对比

#### J. 严格标签告警面板
- **设计要求**：未注册标签阻断时展示详细信息与修复指引（设计 §3.2.2）
- **实际状态**：无

#### K. 冲突矩阵可视化
- **设计要求**：矩阵形式展示跨分片引用关系（设计 §3.4.5）
- **实际状态**：无

#### L. 权限变更矩阵
- **设计要求**：对象 × 权限类型 grid，带色标（设计 §2.1）
- **实际状态**：仅有的 permissions tab 是扁平列表，无矩阵展示

#### M. 在线影响评估报告 UI
- **设计要求**：完整报告含锁范围、耗时预估、OSC 参数预览、归档策略预览（设计 §3.7.3）
- **实际状态**：ImpactReport 在 backend 已实现但前端未消耗

#### N. OSC 工具状态展示
- **设计要求**：gh-ost/pt-osc 进度条、状态指示（设计 §3.7.2）
- **实际状态**：无

#### O. 配置热重载 UI
- **设计要求**：配置变更即时推送（设计 §3.10）
- **实际状态**：无

#### P. 正向/反向执行计划并列对比
- **设计要求**：正向与反向执行计划并列展示（设计 §2.1）
- **实际状态**：仅 SQL 文本切换
